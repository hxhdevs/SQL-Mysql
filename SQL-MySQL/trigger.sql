...
CREATE DATABASE IF NOT EXISTS `curso` /*!40100 DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci */;
USE `curso`;

-- Volcando estructura para disparador curso.productos_after_update
...
CREATE DEFINER=`root`@`localhost` TRIGGER `productos_after_update` AFTER UPDATE ON `productos` FOR EACH ROW BEGIN
	IF NEW.Prod_Precio <> OLD.Prod_Precio THEN
		INSERT INTO productos_historial(PH_Prod_Id, PH_PrecioANT,PH_PrecioNEW,PH_Usuario) VALUES
